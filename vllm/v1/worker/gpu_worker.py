...
class Worker(WorkerBase):
    ...
    def compile_or_warm_up_model(self) -> float:
        ...
        with profiler.track("compile_or_warm_up_model", "total"):
            if self.vllm_config.compilation_config.mode == CompilationMode.VLLM_COMPILE:
                compile_sizes = self.vllm_config.compilation_config.compile_sizes
                warmup_sizes = (
                    compile_sizes.copy() if compile_sizes is not None else []
                )
            ...
            with profiler.track("compile_or_warm_up_model", "warmup_sizes_loop"):
            ...
            with profiler.track("compile_or_warm_up_model", "kernel_warmup"):
                kernel_warmup(self)

            cuda_graph_memory_bytes = 0
            if not self.model_config.enforce_eager:
                with profiler.track("compile_or_warm_up_model", "capture_model"):
            ...
            if self.cache_config.kv_cache_memory_bytes is None and hasattr(
                ...
                msg = (
                    ...
                    f"to fit into requested memory, or `--kv-cache-memory="
                    f"{kv_cache_memory_bytes_to_gpu_limit}` "
                    f"({format_gib(kv_cache_memory_bytes_to_gpu_limit)} GiB) "
                    f"to fully utilize gpu memory. Current kv cache memory in use is "
                    f"{format_gib(self.available_kv_cache_memory_bytes)} GiB."
                )

                logger.debug(msg)

            with profiler.track("compile_or_warm_up_model", "sampler_warmup"):
                if self.use_v2_model_runner:
                    warmup_kernels(
